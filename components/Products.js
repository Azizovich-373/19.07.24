import { deleteData } from "../js/request.http.js"
import { patchedData } from "../js/request.http.js"
...
export function Product(item, arr) {
    ...
    img_basket.onclick = async () => {
        let idx = arr.indexOf(item);
        arr.splice(idx, 1);
        tr.remove();

        await deleteData(`/todos/${item.id}`, tr)
    }
    ...
    img_pererabotka.onclick = async () => {
        patchedData(`/todos/${item.id}`, {complet: !item.complet})
            .then(res => {
